...
int main()
{
    ...
    */
    ...
    int totalCount = 0; // how many is divisible?

    for(int i = numOfDigits; i>0; i--)  // eg 1234 i=4 234 i=3 34 i=2 4 i=1
    {
        int sstring = inputNumber;
        sstring %= static_cast<int>(pow(10,i)); // mod 

        for(int j=0 ; j < i ; j++) //  j < digit of sstring  ,get quotient  eg1234 1234 123 12 1 
        {
            int ssstring= static_cast<int>(sstring / pow(10,j));
            if((inputNumber % ssstring )==0) // check divisibility
            {
                cout << ssstring << endl;
                totalCount++;
            }
